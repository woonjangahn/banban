...
export function CommentItem({ 
  ...
  const [upvotes, setUpvotes] = useState(comment.reactions?.filter(r => r.reaction_type === 'upvote').length || 0);
  const [downvotes, setDownvotes] = useState(comment.reactions?.filter(r => r.reaction_type === 'downvote').length || 0);
  ...
  const handleReplySubmit = useCallback((newComment: Comment) => {
